...
int mem_fill(void *buf, unsigned long start, long len,
        unsigned char *pat_val, int pat_len, int blen)
{
  ...
  volatile unsigned long* p4buf= (unsigned long*)buf;
  volatile unsigned short* p2buf= (unsigned short*)buf;
  volatile unsigned char* p1buf= (unsigned char*)buf;
...
usage(void)
{
  printf("usage: mfill -b <location> -l <length> -p <pattern> [-1|-2|-4]\n");
  printf("  -b, --base  <num>        base address of transfer\n");
  printf("  -l, --length <num>       length of transfer\n");
  printf("  -p, --pattern <num>      data pattern\n");
...
int main(int argc, char *argv[])
{
  ...
  while ((opt = getopt_long(argc, argv, ":b:l:p:h124",
  ...
  switch (opt) {
    ...
    case 'p':
      fill_flg=1;
      mem_data=strtoul(optarg,NULL,0);
      write_flg=1;
      break;
    case 'b':
...

  {
    ...
    */
    ...
    int unmap = munmap(mm, mem_window_size);
